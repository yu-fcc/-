...
type DoctorRegisterController struct {
	beego.Controller
}

func (c *DoctorRegisterController) Get() {
	c.TplName = "doctorRegister.html"
}

// 前端通过后端请求地址/dodregister来调用该方法，获取前端的信息，将该信息插入到Doctorinfo表中
func (c *DoctorRegisterController) Register() {
	...
	// 生成密码哈希
	passwordHash, err := bcrypt.GenerateFromPassword([]byte(doctor.Upwd), bcrypt.DefaultCost)
	if err != nil {
	...
	doctor.Upwd = string(passwordHash)
	om := orm.NewOrm()
	id, err := om.Insert(&doctor)
